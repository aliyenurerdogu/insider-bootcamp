...
class ShoppingCart {
    ...
    addItem(productId, quantity = 1) {
        ...
        try {
            ...
            product.stock -= quantity; // HATA DÜZELTİLDİ: Stok doğru şekilde azaltıldı.
    ...
    removeItem(productId) {
        ...
        try {
            ...
            if (product) {
                product.stock += item.quantity; // HATA DÜZELTİLDİ: Stok iade edilirken sabit değer yerine item.quantity kullanıldı
            }

            this.items.splice(itemIndex, 1);
            this.calculateTotal();
            this.updateUI();
            document.dispatchEvent(new Event('stockUpdate')); // Ürün listesi güncellendi
    ...
    calculateTotal() {
        this.total = this.items.reduce((sum, item) => {
            return sum + (item.price * item.quantity); // HATA DÜZELTİLDİ: Miktar hesaba katıldı
        }, 0);

        if (this.discountApplied && this.total > 0) {
            this.total *= 0.9; // HATA DÜZELTİLDİ: Doğru indirim uygulandı
        }
    }
    ...
    showError(message) {
        ...
        if (errorElement) {
            errorElement.textContent = message; //errorElement.textContent += message + '\n'; satırı ile her hatada önceki mesajlar silinmeden yeni hata ekleniyodu.
        }
        setTimeout(() => {
            errorElement.textContent = ''; //3 saniye sonrayı hatayı temizle
        }, 3000);
    }
